...
from storage.storage.storage_template import find_, insert_one, update_one, update_, find_one
...
def update_monitor_rule(monitor_rule: MonitorRule):
    if monitor_rule.factorId is None:
        print("topic ")
        return update_({"and":[{"topicId":monitor_rule.topicId},{"code":monitor_rule.code}]},monitor_rule, MonitorRule, MONITOR_RULES)
    else:
        print("factor ")
        return update_({"and": [{"topicId": monitor_rule.topicId}, {"code": monitor_rule.code},{"factorId":monitor_rule.factorId}]}, monitor_rule,
                       MonitorRule, MONITOR_RULES)
