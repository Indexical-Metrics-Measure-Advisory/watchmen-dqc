...
class FactorAvgNotInRange(AbstractRule):
    ...
    def execute(self) -> bool:
        ...
        if self.rule.params.min and self.rule.params.max:
            result = self.match_factor_avg_not_in_range()
            return not (self.rule.params.min <= result["avg"] <= self.rule.params.max)

    def get_factor_avg_not_in_range_sql(self) -> str:
        sql = "SELECT avg({field}) as avg " \
              "FROM {schema}.{table} " \
    ...
    def match_factor_avg_not_in_range(self) -> Dict:
        query_sql = self.get_factor_avg_not_in_range_sql()
        log.info("factor avg not in range: {sql}".format(sql=query_sql))
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute(query_sql)
        row = cursor.fetchone()
        result = self.get_avg_result(row)
        return result

    @staticmethod
    def get_avg_result(row):
        result = {}
        if row:
            for index, value in enumerate(row):
                if index == 0:
                    result["avg"] = Decimal(value)
        return result
