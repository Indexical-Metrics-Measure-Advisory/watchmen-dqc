...
class FactorIsEmpty(AbstractRule):
    ...
    def execute(self) -> bool:
        super().execute()
        match_result = self.match_factor_is_empty()
        return match_result["count"] != 0

    def get_factor_is_empty_sql(self) -> str:
    ...
    def match_factor_is_empty(self) -> Dict:
        sql = self.get_factor_is_empty_sql()
        log.info("factor is empty: {sql}".format(sql=sql))
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute(sql)
        row = cursor.fetchone()
